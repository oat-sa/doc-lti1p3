...
**Note**: if you want to work with the message payload claims as array of values (for caching purposes for example), you can use the [MessagePayloadClaimsExtractor](https://github.com/oat-sa/lib-lti1p3-core/blob/master/src/Message/Payload/Extractor/MessagePayloadClaimsExtractor.php) for extracting claims (provides a configurable claim sanitization).
